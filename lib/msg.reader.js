...
(function(root, factory) {
  ...
  function headerData(ds) {
    ...
    headerData.propertyStart = ds.readInt(
      CONST.MSG.HEADER.PROPERTY_START_OFFSET
    );
  ...
  function convertBlockToProperties(ds, msgData, propertyBlockOffset, props) {
    ...
    for (var i = 0; i < propertyCount; i++) {
      var propertyType = ds.readByte(
        propertyOffset + CONST.MSG.PROP.TYPE_OFFSET
      );
      switch (propertyType) {
  ...
  function fieldsDataDirInner(ds, msgData, dirProperty, fields) {
    if (dirProperty.name.indexOf(CONST.MSG.FIELD.PREFIX.ATTACHMENT) == 0) {
      ...
      dirProperty.name.indexOf(CONST.MSG.FIELD.PREFIX.RECIPIENT) == 0
    ) {
      // recipient
      var recipientField = {};
  ...
  function fieldsDataDocument(ds, msgData, documentProperty, fields) {
    ...
    if (fieldName) {
      if (fieldName === "body") {
        if (
          typeof getFieldValue(ds, msgData, documentProperty, fieldType) ===
          "string"
        )
          fields[fieldName] = getFieldValue(
            ds,
            msgData,
            documentProperty,
            fieldType
  ...
  MSGReader.prototype = {
    ...
    getFileData: function() {
      ...
      if (this.fileData == null) {
        this.fileData = SendTypeSort(parseMsgData(this.ds));
      }
...
function SendTypeSort(FileData) {
  const returnFileData = JSON.parse(JSON.stringify(FileData));
  let cc = returnFileData.fieldsData.cc || "";
  let bcc = returnFileData.fieldsData.bcc || "";
  for (
    let index = 0;
    index < returnFileData.fieldsData.recipients.length;
    index++
  ) {
    const user = returnFileData.fieldsData.recipients[index];
    let sendType = "to";
    if (~cc.indexOf(user.name) || ~cc.indexOf(user.email)) {
      sendType = "cc";
      cc = cc.split(user.name).join("");
      cc = cc.split(user.email).join("");
    } else if (~bcc.indexOf(user.name) || ~bcc.indexOf(user.email)) {
      sendType = "bcc";
      bcc = bcc.split(user.name).join("");
      bcc = bcc.split(user.email).join("");
    }
    returnFileData.fieldsData.recipients[index].sendType = sendType;
  }
